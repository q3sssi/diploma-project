...
def query_table(table_name: str, limit: int = 500):
    """Возвращает данные таблицы для построения чартов в React"""
    try:
        conn = pg_connect()
        cur = conn.cursor()
        ...
        result = []
        for row in rows:
            result.append({cols[i]: (float(v) if hasattr(v, '__float__') and not isinstance(v, str) else str(v) if v is not None else None)
                          for i, v in enumerate(row)})
        return {"rows": result, "columns": cols, "total": len(result)}
    except Exception as e:
        raise HTTPException(500, str(e))

@app.post("/api/superset/guest-token")
async def get_guest_token(body: dict):
    """Генерирует Guest Token для Embedded Superset"""
    dashboard_id = body.get("dashboard_id")
    if not dashboard_id:
        raise HTTPException(400, "Укажите dashboard_id")
    try:
        # Получаем обычный токен
        token = superset_token()
        headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}

        # Генерируем Guest Token
        payload = {
            "user": {"username": "guest", "first_name": "Guest", "last_name": "User"},
            "resources": [{"type": "dashboard", "id": str(dashboard_id)}],
            "rls": []
        }
        r = requests.post(f"{SUPERSET_URL}/api/v1/security/guest_token/",
                          headers=headers, json=payload, timeout=15)
        r.raise_for_status()
        return {"token": r.json()["token"]}
    except Exception as e:
        raise HTTPException(500, f"Ошибка генерации Guest Token: {e}")

@app.get("/api/superset/dashboards")
def get_dashboards():
    """Список дашбордов из Superset"""
    try:
        token = superset_token()
        r = requests.get(f"{SUPERSET_URL}/api/v1/dashboard/",
                         headers={"Authorization": f"Bearer {token}"}, timeout=10)
        r.raise_for_status()
        dashboards = r.json().get("result", [])
        return {"dashboards": [
            {"id": d["id"], "uuid": d.get("uuid"), "title": d.get("dashboard_title", f"Dashboard {d['id']}"), "published": d.get("published", False)}
            for d in dashboards
        ]}
    except Exception as e:
